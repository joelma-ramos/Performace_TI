...
function StandardSlide({ slide }: { slide: SlideData }) {
  // Classic Consulting patterns
  const isAnalysis = slide.type === 'analysis';
  const isComparison = slide.id === 10 || slide.id === 11;
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div className="space-y-4 max-w-3xl">
          <div className="flex items-center gap-3">
             ...
             <span className="font-mono text-[10px] text-brand-gray tracking-[0.3em] uppercase font-bold">Resumo Estratégico</span>
          </div>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-display font-black text-white leading-[1.1] tracking-tight">
            {slide.title}
          </h2>
        </div>
        <div className="hidden md:block">
           <div className="px-5 py-2 rounded-full border border-white/10 bg-white/5 flex items-center gap-3">
              <slide.icon className="w-4 h-4 text-brand-accent" />
              <span className="text-[10px] font-mono font-bold text-brand-gray tracking-widest">{slide.id.toString().padStart(2, '0')}</span>
           </div>
        </div>
      </div>

      {/* Main Content: Bento Grid Layout */}
      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {slide.bullets.map((bullet, idx) => {
          ...
          return (
            <motion.div 
              ...
              className="group p-6 rounded-2xl bg-white/[0.03] border border-white/5 hover:bg-white/[0.06] hover:border-brand-accent/20 transition-all card-gradient flex flex-col justify-between"
            >
              <div className="space-y-4">
                <div className="w-8 h-8 rounded-lg bg-brand-accent/5 flex items-center justify-center group-hover:bg-brand-accent/20 transition-colors">
                   <div className="w-1.5 h-1.5 rounded-full bg-brand-accent" />
                </div>
                <div className="space-y-2">
                  <h4 className="text-brand-accent font-display font-bold text-sm uppercase tracking-wider">{label}</h4>
                  {desc.length > 0 && (
                    <p className="text-brand-silver/70 text-sm leading-relaxed font-light">{desc.join(':').trim()}</p>
                  )}
                </div>
              </div>
              <div className="mt-6 h-[2px] w-0 group-hover:w-full bg-brand-accent/40 transition-all duration-500" />
            </motion.div>
          );
        })}
      </div>

      {/* Bottom Section: Executive Insight Box */}
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.8 }}
        className="mt-auto p-6 rounded-2xl border border-brand-accent/10 bg-brand-accent/[0.02] flex items-start gap-5 relative overflow-hidden"
      >
        <div className="absolute top-0 right-0 p-3 opacity-5">
           <Target className="w-16 h-16 text-brand-accent" />
        </div>
        <div className="mt-1">
          <div className="w-10 h-10 rounded-full bg-brand-accent flex items-center justify-center shadow-[0_0_20px_rgba(17,239,172,0.3)]">
             <Zap className="w-5 h-5 text-brand-blue" />
          </div>
        </div>
        <div className="space-y-1">
           <span className="text-[10px] font-mono font-bold text-brand-accent uppercase tracking-[0.2em]">Insight Executivo</span>
           <p className="text-white text-base md:text-lg font-medium leading-tight">
             {slide.script.split('.')[0]}.
           </p>
        </div>
      </motion.div>
    </div>
